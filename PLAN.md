...
1. **P9 性能优化先做**：保证 Quest 演示更稳，优先处理卡顿、掉帧、画面杂乱的问题
2. **P10 演示包装跟进**：继续打磨镜头、UI、音频、节奏，让整段演示更像完整作品
3. **训练模式只保留现状**：P11 当前先停在 Mock 可展示状态，不再投入真机 BLE 联调
4. **复杂真机链路后移**：BS-BT91、BLE 协议、Quest 真机数据接入统一延后，避免影响演示主线

### 后续

- **P9 性能优化**：Quest 稳定帧率、减少 Draw Call、控制 APK 体积
- **P10 演示视频**：等画面、节奏、音频稳定后录屏
- **P11 智能足球**：暂缓，等演示主线收口后再决定是否恢复
