...
当前默认进入键鼠优先的 `Arena Attack`：机器人连续发球，玩家用第三人称视角完成停球、传球、射门和抢断；球撞场边会回弹，90 秒内尽量多进球。原来的接球、Recovery、回放与评分训练流程继续保留，可用 `F8` 重载切换。
...
- [Arena Attack 操作说明与已知问题](docs/ARENA_ATTACK_OPERATION_GUIDE.md)
- [核心玩法重构](docs/CORE_GAMEPLAY_REWORK.md)
- [核心玩法 V1.2 演示反馈](docs/CORE_GAMEPLAY_REWORK_V1_2_DEMO_FEEDBACK.md)
- [LLM AI Coach 集成文档](docs/LLM_AI_COACH_INTEGRATION.md)
- [演示画面与美术优化确认](docs/DEMO_VISUAL_ART_OPTIMIZATION_PLAN.md)
- [Quest 手柄当腿脚的交互设计](docs/QUEST_CONTROLLER_LEG_INTERACTION_DESIGN.md)

## 输入方式

Arena 默认键鼠：`WASD` 移动、鼠标视角、`Left Shift` 冲刺、`E` 停球、`RMB` 传球、`LMB` 射门、`Space` 抢断、`Esc` 暂停。`F7` 轮换控制档，`F8` 切换 Training/Arena，`F9` 切调试面板。
...
3. 在 Editor 中 Play；默认自动启动 Arena，无需头显或真实机器人。
4. 用键鼠完成停球、传射和抢断；进球、被断或卡球后机器人会自动重新发球。
5. 用 `F8` 切回 Training 后，可继续验证旧的接球、Recovery、回放与评分流程。
6. 用 `F7` 切到 Gamepad 或 XrSimulator；XrSimulator 档会在 Editor Play Mode 自动创建项目已有的 XR Device Simulator prefab。
...
| 测试 | Unity Test Framework，当前有接球、Recovery、Field AI 与 Arena 规则 EditMode 测试 |
...
| 脚本 | 职责 |
|------|------|
| [GameplayModeBootstrap.cs](unity/Assets/Scripts/Flow/GameplayModeBootstrap.cs) | 默认模式/控制档、命令行参数和 Training/Arena 重载切换 |
| [ArenaAttackController.cs](unity/Assets/Scripts/Flow/ArenaAttackController.cs) | 90 秒 Arena 回合、边墙/球门、机器人发球、轻量 AI、HUD 与调试面板 |
| [ArenaPlayerController.cs](unity/Assets/Scripts/Player/ArenaPlayerController.cs) | 第三人称键鼠/手柄输入、移动、相机、蓄力和五个核心动作 |
| [ArenaBallMotor.cs](unity/Assets/Scripts/Ball/ArenaBallMotor.cs) | 物理控球辅助、传射/抢断冲量、争抢状态与 VR 实体脚动作桥接 |
| [MatchFlowController.cs](unity/Assets/Scripts/Flow/MatchFlowController.cs) | 比赛阶段、传球、接球质量、Recovery、脚部触球路由和结果判断 |
| [FieldAIController.cs](unity/Assets/Scripts/Flow/FieldAIController.cs) | 队友接应、对手压迫/截传、门将跟球与扑救状态 |
| [NpcAnimationPresenter.cs](unity/Assets/Scripts/Flow/NpcAnimationPresenter.cs) | 按实际移动速度驱动 NPC idle/walk/run，并提供门将侧扑与无 Animator 动态兜底 |
